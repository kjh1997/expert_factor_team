...
class run_factor_integration:
    ...
    def factor_norm(self):
        ...
        self.ID['test'].update_many({"keyId":self.keyid, "fid":self.fid}, [{"$set" : {"factor.coop":{"$multiply": ["$factor.coop", real_coop]},"score" : {'$sum':[{"$multiply": ["$factor.qual", real_qual, 25]}, {"$multiply": ["$factor.acc", 25]},{"$multiply": ["$factor.coop", real_coop, 25]},{"$multiply": ["$factor.qunt", 25]}]}}}])
        print("종료")
