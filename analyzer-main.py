...
class Analysis(threading.Thread):
    ...
    def run(self):
        ...
        tempQual = -1
        tempCoop = -1
        maxFactors = {'Quality' : -1, 'Productivity' : -1, 'Contrib' : -1 } #초기화
        ...
        for i in range( allPage):
         ...
         #return pYears, keywords, {'issueInsts' : issueInsts, 'issueLangs' : issueLangs, 'citation' : citation}, {'authors' : authors, 'A_ID' : A_ID  }, authorInsts
            if len(pYears) > 0 :
                contrib  = self.cont(_contBackdata)
